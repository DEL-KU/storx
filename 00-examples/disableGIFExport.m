...
disp('GIF export is truned off for all examples. You can export GIFs by setting exportGIF = 1;');
end
...
for k = 1:length(files)
    ...
    if strcmp(files(k).name, 'disableGIFExport.m') || strcmp(files(k).name, 'enableGIFExport.m')
    ...
    pattern = '\<exportGIF\>\s*=\s*(?:true|0)\s*;?';  % word-boundary safe in MATLAB
    replacement = 'exportGIF = false;';
    ...
    if changed
        ...
        if fid == -1
            ...
            fclose(fid);
            fprintf('Processed file: %s - exportGIF disabled\n', filePath);
        end
    else
        fprintf('Processed file: %s - exportGIF already disabled or not found\n', filePath);
    end
