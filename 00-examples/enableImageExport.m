...
disp('Image export is truned on for all examples.');
end
...
for k = 1:length(files)
    ...
    if strcmp(files(k).name, 'disableImageExport.m') || strcmp(files(k).name, 'enableImageExport.m')
    ...
    pattern = '\<exportImages\>\s*=\s*(?:false|0)\s*;?';  % word-boundary safe in MATLAB
    replacement = 'exportImages = true;';
    ...
    if changed
        ...
        if fid == -1
            ...
            fclose(fid);
            fprintf('Processed file: %s - exportImages enabled\n', filePath);
        end
    else
        fprintf('Processed file: %s - exportImages already enabled or not found\n', filePath);
    end
