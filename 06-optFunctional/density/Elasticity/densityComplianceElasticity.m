...
classdef densityComplianceElasticity < functional
    ...
    methods
        %% CONSTRUCTOR
        function obj = densityComplianceElasticity(solver, ub)
                ...
                for scenarioId = 1:obj.m_solver.m_numScenarios
                    for ely = 1:obj.m_solver.m_ny
                        for elx = 1:obj.m_solver.m_nx
                            ...
                            n2 = (obj.m_solver.m_ny+1)* elx   +ely;
                            Ue = U([2*n1-1;2*n1; 2*n2-1;2*n2; 2*n2+1;2*n2+2; 2*n1+1;2*n1+2],scenarioId);
                            Fbody = 1/obj.m_x(ely,elx)*obj.m_solver.m_fBody([2*n1-1;2*n1; 2*n2-1;2*n2; 2*n2+1;2*n2+2; 2*n1+1;2*n1+2],scenarioId);
                            x = X(ely,elx);
                            interpCoeffGrad = obj.m_solver.getInterpolationCoefficientGrad(x);

                            obj.m_dfdx(ely,elx,scenarioId) = ...
                                -interpCoeffGrad*Ue'*KE*Ue + Fbody'*Ue;
                        end
                    end
                end
            else
