...
if (buildPhi)
    ...
    % Lift objective (Brinkman body force in x2-direction)
    phi = -alpha*ux(2);
...
if (exportPhi)
    fprintf('Exporting phi_lift... \n');
    f = matlabFunction(phi,'File','PHI_lift','Comments','Version: 0.9','Vars',{dx,dy,mu,alpha,transpose([u1 u2 u3 u4 u5 u6 u7 u8 p1 p2 p3 p4])});
    fprintf('Exporting dphi_lift/dg... \n');
    f = matlabFunction(dphidg,'File','dPHIdg_lift','Comments','Version: 0.9','Vars',{dx,dy,mu,alpha,dalpha,transpose([u1 u2 u3 u4 u5 u6 u7 u8 p1 p2 p3 p4])});
    fprintf('Exporting dphi_lift/ds... \n');
    f = matlabFunction(dphids(1:8),'File','dPHIds_lift','Comments','Version: 0.9','Vars',{dx,dy,mu,alpha,transpose([u1 u2 u3 u4 u5 u6 u7 u8 p1 p2 p3 p4])});
end
if (exportJR)
    fprintf('Exporting dr/ds... \n');
    f = matlabFunction(drdg,'File','dRESdg_lift','Comments','Version: 0.9','Vars',{dx,dy,mu,rho,alpha,dalpha,transpose([u1 u2 u3 u4 u5 u6 u7 u8 p1 p2 p3 p4])});
end
%%
time = toc(time);
