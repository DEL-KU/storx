...
classdef modifiedHJComplianceElasticity < functional
    properties (GetAccess = 'public', SetAccess = 'protected')
        m_ce; % compliance at every element

        m_lambda;
        m_mu;
        m_Ktr; % the "trace" matrix
    end

    methods
        %% CONSTRUCTOR
        function obj = modifiedHJComplianceElasticity(solver, ub)
            ...
            E = obj.m_solver.m_materials(1).E;
            nu = obj.m_solver.m_materials(1).nu;
            obj.m_lambda = E*nu/((1+nu)*(1-nu)); % Lame' parameters
            obj.m_mu = E/(2*(1+nu));
            obj = obj.computeKtr();
        end
        ...
        function [obj,grad] = gradient(obj)
            ...
            dTdx = zeros(obj.m_solver.m_ny,obj.m_solver.m_nx,obj.m_solver.m_numScenarios);
            U = obj.m_solver.m_sol;
            ...
            for scenarioId = 1:obj.m_solver.m_numScenarios
                for ely = 1:obj.m_solver.m_ny
                    for elx = 1:obj.m_solver.m_nx
                        ...
                        dTdx(ely,elx,scenarioId) = X(ely,elx)*...
                            pi/2*(obj.m_lambda+2*obj.m_mu)/obj.m_mu/(obj.m_lambda+obj.m_mu)*...
                            (4*obj.m_mu*Ue'*obj.m_solver.m_KE*Ue+(obj.m_lambda-obj.m_mu)*Ue'*obj.m_Ktr*Ue);
                    end
                end
            end
            ...
            dTdx = mean(dTdx,3); % take average of all scenarios
            dTdx = dTdx/sum(obj.m_fx0);

            obj.m_dfdx.dSdx = dSdx; % shape sensitivity
            obj.m_dfdx.dTdx = dTdx; % topology sensitivity

            if (obj.m_isConstraint)
                grad.dSdx = obj.m_dfdx.dSdx/obj.m_upperBound;
                grad.dTdx = obj.m_dfdx.dTdx/obj.m_upperBound;
            else
                grad = obj.m_dfdx;
            end
        end
    ...
    methods (Access = private)
        function obj = computeKtr(obj)
            % compute the "trace" matrix
            E = obj.m_solver.m_materials(1).E;
            nu = obj.m_solver.m_materials(1).nu;
            ve = obj.m_solver.m_hx*obj.m_solver.m_hy;
            ...
            if (obj.m_solver.m_vectorize==1)
                order = [7 5 3 1 8 6 4 2]; % alternate u and v
                obj.m_Ktr(:,:) = obj.m_Ktr(order,order);
            end
        end
