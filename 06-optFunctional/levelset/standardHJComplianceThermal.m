...
classdef standardHJComplianceThermal < functional
    ...
    methods
        %% CONSTRUCTOR
        function obj = standardHJComplianceThermal(solver, ub)
            % check if solver is valid
            if (~isa(solver, 'fea2d_thermal')), error('solver must be an instance of fea2d_thermal class!');end
        ...
        % evaluate the compliance value at design and state variables queried from the thermal solver
        % input: obj
        ...
        function [obj,value] = evaluate(obj)
            ...
            if (obj.m_solver.m_vectorize==0)
                ...
                for scenarioId = 1:obj.m_solver.m_numScenarios
                    for ely = 1:obj.m_solver.m_ny
                        for elx = 1:obj.m_solver.m_nx
                            ...
                            Te = T([n1; n2;n2+1;n1+1],scenarioId);
                            obj.m_fx(scenarioId)  = obj.m_fx(scenarioId)  + X(ely,elx)*Te'*obj.m_solver.m_KE*Te;
                        end
                    end
                end
            else
                ...
                for scenarioId = 1:obj.m_solver.m_numScenarios
                    T = obj.m_solver.m_sol(:,scenarioId);
                    obj.m_ce(:,:,scenarioId) = reshape(sum((T(obj.m_solver.m_edofMat)*KE).*T(obj.m_solver.m_edofMat),2),obj.m_solver.m_ny,obj.m_solver.m_nx);
        ...
        function [obj,grad] = gradient(obj)
            ...
            for scenarioId = 1:obj.m_solver.m_numScenarios
                for ely = 1:obj.m_solver.m_ny
                    for elx = 1:obj.m_solver.m_nx
                        if (~obj.m_solver.m_solidElems(ely,elx)), continue; end
                        n1 = (obj.m_solver.m_ny+1)*(elx-1)+ely;
                        n2 = (obj.m_solver.m_ny+1)* elx   +ely;
                        Te = T([n1; n2;n2+1;n1+1],scenarioId);
                        dSdx(ely,elx,scenarioId) = -max(X(ely,elx),1e-9)*Te'*obj.m_solver.m_KE*Te;
                    end
                end
            end
