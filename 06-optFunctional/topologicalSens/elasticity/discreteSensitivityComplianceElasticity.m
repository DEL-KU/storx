...
% gradient using discrete sensitivity fieldsfor evolutionary and            %
...
classdef discreteSensitivityComplianceElasticity < functional
    ...
    methods
        %% CONSTRUCTOR
        function obj = discreteSensitivityComplianceElasticity(solver, ub)
        ...
        function [obj,grad] = gradient(obj)
            %  Compute the discrete topological sensitivity at the
                ...
                for scenarioId = 1:obj.m_solver.m_numScenarios
                    for  elx = 1:obj.m_solver.m_nx
                        for  ely = 1:obj.m_solver.m_ny
                            ...
                            n2 = (obj.m_solver.m_ny+1)* elx   +ely;
                            Ue = U([2*n1-1;2*n1; 2*n2-1;2*n2; 2*n2+1;2*n2+2; 2*n1+1;2*n1+2],scenarioId);
                            obj.m_dfdx(ely,elx,scenarioId) = Ue'*obj.m_solver.m_KE*Ue;
                        end
                    end
                end
            else
                obj.m_dfdx = X.*obj.m_ce;
            end
