...
if iter < 3
  ...
  low = max(low, xval - 10*xmami);
  low = min(low, xval - 1e-2*xmami);
  upp = max(upp, xval + 1e-2*xmami);
  upp = min(upp, xval + 10*xmami);
  xmi = xmin - 1e-6;
  xma = xmax + 1e-6;
  l_id = xval < xmi;
  g_id = xval > xma;
  low(l_id) = xval(l_id) - (xma(l_id) - xval(l_id))/0.9;
  upp(l_id) = xval(l_id) + (xma(l_id) - xval(l_id))/0.9;
  low(g_id) = xval(g_id) - (xval(g_id) - xmi(g_id))/0.9;
  upp(g_id) = xval(g_id) + (xval(g_id) - xmi(g_id))/0.9;
...
%%
PQ = 0.001*abs(P + Q) + raa0*eeem*xmamiinv';
P = max(dfdx,0);
Q = max(-dfdx,0);
