...
%   curve0 = g.xs{2};
curve0=g.xs{2}-0.15;
%   curve0= sqrt((g.xs{1}-0).^2 + (g.xs{2}-0).^2) - 1;
%   curve0_tem = g.xs{2};
%    curve0_a= g.xs{2}+2;
%    curve0=max(curve0_tem, -curve0_a);
%    curve0_a= g.xs{2}-2;
%     curve0=min(curve0, -curve0_a);
  mask0 = 0.1 - abs(g.xs{1}-0.5);
%   mask0_a = 4 - abs(g.xs{1});
%    mask0 = 6 - abs(g.xs{1});
%    mask0=min(mask0, -mask0_a);
 case 7,
  lambda = 0.2;
  tLast = 25;
  tPlot = [ 1, 8, 16, 25 ];
 ...
 case 12,
  ...
  tLast = 25;
  tPlot = [ 1, 7, 16, 25 ];
  curve0 = -min(0.5 + g.xs{2},0.5 - g.xs{2});  %%1
  % The equation (21) for this function was buggy in Smereka.
%   mask0 = max((g.xs{1} - 0.5)/1 + g.xs{2}, (0.5 - g.xs{1})/1 - g.xs{2} - 0.25);  % 
...
%---------------------------------------------------------------------------
% Set up time approximation scheme.e
...
% schemeData may contain other fields.
  ...
  speed = 6*thisSchemeData.speedConst .* ...
          feval(thisSchemeData.maskFunc, data{2}, thisSchemeData.epsilon); %%%6
