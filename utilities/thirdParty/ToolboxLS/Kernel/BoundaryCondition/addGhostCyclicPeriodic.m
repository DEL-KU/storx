function dataOut = addGhostCyclicPeriodic(dataIn, dim, width, ghostData)
% addGhostCyclicPeriodic: add ghost cells with cyclic periodic boundary conditions.
%by Qian Ye 3/9/2020
...
% end  
  indicesOut{1} = 1:sizeIn(2);   % indicate the number of nodes on x;
 
   indicesOut{2} = 1:sizeIn(1);   %indicate the number of nodes on y;
indicesIn = indicesOut;

% create appropriately sized output array
sizeOut = sizeIn;
sizeOut(dim) = sizeOut(dim) + 2* width;
% sizeOut=flip(sizeOut);
...
dataOut(indicesOut{:}) = flip(dataInC(indicesIn{:}));
...
% dataOut=reshape(aaa,size(dataOut));
