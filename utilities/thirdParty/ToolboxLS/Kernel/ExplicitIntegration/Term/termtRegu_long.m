function [ ydot, stepBound, schemeData ] = termtRegu_long(t, y, schemeData)
...
% Updated to include passVLS option.  Ian Mitchell 02/14/05.
  ...
  %%%%%Long Add
  DeriveL = zeros(size(data));
  DeriveR = zeros(size(data));
  %%%%% Long Add end
  ...
  for i = 1 : grid.dim
    ...
    DeriveL = derivL.^2 .* flowL;
    DeriveR = derivR.^2 .* flowR;
  ...
  % magnitude is |\grad \phi\|, namely the "s" in DRLSE
  magnitude = sqrt(magnitude);
  
  %%%%%% Long Edit
  y=reshape(y,101,201);
 [phi_x,phi_y]=gradient(y);
  ...
  f = div(dps.*phi_x - phi_x, dps.*phi_y - phi_y) + 4*del2(y);
  f=reshape(f,20301,1);
  ydot=0.01*f;
  %%%%%% End 
