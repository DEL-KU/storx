function [ Phi, g ] = LevelSetReinit2D(Phi, g)
%
% [ Phi] = LevelSetReini(Phi,NodeNumPerRow,NodeNumPerCol,nodesPosition,DetX, accuracy, displayType)
%
...
run('addPathToKernel');
disp('Reinitialization begins!');
%---------------------------------------------------------------------------
% Integration parameters.
tMax =1*g.dx(1);                  % End time.
% tMax = g.dx(1);
tPlot = 2;                 % Period at which plot should be produced.
t0 = 0;                      % Start time.
...
% 
% figure;
% contourf(g.xs{1},g.xs{2},data,[0 0]);
% axis equal;
% grid on;
% % hold on;
% % contour(g.xs{1},g.xs{2},-data0,[0 0]);
% % axis equal;

Phi = reshape(y, size(Phi));

disp('Reinitialization ends!');
disp(' ');